namespace CentileData {
    export class Lms {
        ...
        readonly M: number;
        readonly S: number;
        constructor(l: number, m: number, s: number) {
            this.L = l;
            this.M = m;
            this.S = s;
        }
        ...
        CumSnormfromParam = function (param: number) {
            return this.CumSNorm(this.ZFromParam(param));
        }

        static CumSNorm(zScore: number){
            let zAbs = Math.abs(zScore);
